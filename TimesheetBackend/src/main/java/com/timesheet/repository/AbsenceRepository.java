...
public interface AbsenceRepository extends JpaRepository<Absence, Integer> {
    ...
    @Query("SELECT new com.timesheet.dto.absence.AbsenceDto(absence.id, absence.reason, absence.employee.id, CONCAT(absence.employee.firstName, ' ', absence.employee.lastName) ,absence.dateRequest, absence.dateSubmit, absence.typeTimeOff, absence.timeOff, absence.status, absence.punishmentStatus) FROM Absence absence WHERE absence.dateRequest = ?1")
